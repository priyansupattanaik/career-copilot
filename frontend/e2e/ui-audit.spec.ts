...
async function auditPage(page: Page, route: string, viewportName: string): Promise<Finding[]> {
  return page.evaluate(
    ({ routeName, vp }) => {
      ...
      for (const el of candidates) {
        if (el.closest("svg, .stars-background-layer, .aurora-background-field, .profile-tabs")) continue;
        const style = getComputedStyle(el);
        if (style.position === "fixed" || style.position === "absolute") continue;
        const r = el.getBoundingClientRect();
        if (r.left < -2 || r.right > vw + 2) {
          const cls = (el as HTMLElement).className?.toString?.().slice(0, 80) || el.tagName.toLowerCase();
          findings.push({
            route: routeName,
            viewport: vp,
            kind: "off-viewport-x",
      ...
      const boxes = layoutSelectors
        ...
        .filter((row): row is { selector: string; x: number; y: number; w: number; h: number } => Boolean(row));
      ...
      const headings = Array.from(document.querySelectorAll("h1, h2, .button, .lp-title, .ra-title"));
      for (const el of headings) {
        const html = el as HTMLElement;
        if (html.scrollWidth > html.clientWidth + 4 && getComputedStyle(html).overflow !== "visible") {
          findings.push({
            route: routeName,
            viewport: vp,
            kind: "text-clip",
      ...
      const fonts = Array.from(
        new Set(
          [document.body, ...Array.from(document.querySelectorAll("h1, h2, h3"))]
...
test("all pages stay aligned across desktop, tablet, and phone", async ({ page }, testInfo) => {
  ...
  for (const viewport of VIEWPORTS) {
    ...
    for (const route of [...PUBLIC_ROUTES, ...WORKSPACE_ROUTES]) {
      ...
      await page.locator(".feature-loading").waitFor({ state: "detached", timeout: 20_000 }).catch(() => undefined);
      await page
        .waitForFunction(() => {
          if (document.querySelector(".feature-loading")) return false;
          return Array.from(document.querySelectorAll("h1, h2, .lp-title, .ra-title")).some((node) => {
      ...
      await page.screenshot({ path: path.join(shotDir, `${safe}.png`), fullPage: true });
  ...
  const unique = findings.filter((row) => row.kind !== "off-viewport-x" || !row.detail.includes("svg"));
  if (unique.length) {
    console.log(`UI_AUDIT_FINDINGS ${unique.length}\n${unique.map((row) => `[${row.viewport}] ${row.route} ${row.kind}: ${row.detail}`).join("\n")}`);
  }
  expect(unique, unique.map((row) => `[${row.viewport}] ${row.route} ${row.kind}: ${row.detail}`).join("\n")).toEqual([]);
});
