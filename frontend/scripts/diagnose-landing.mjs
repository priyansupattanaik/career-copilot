...
const baseUrl = process.argv[2] || process.env.PLAYWRIGHT_BASE_URL || "http://127.0.0.1:3000";
const outDir = join(dirname(fileURLToPath(import.meta.url)), "..", "test-results", "landing-diagnose");
...
async function collectPageSignals(page, label) {
  const signals = await page.evaluate(() => {
    ...
    const luminance = (rgb) => {
      const channels = rgb.map((c) => c / 255).map((c) => (c <= 0.03928 ? c / 12.92 : ((c + 0.055) / 1.055) ** 2.4));
    ...
    const boxes = selectors.flatMap((selector) =>
      Array.from(document.querySelectorAll(selector)).map((el, index) => {
        ...
        return {
          ...
          overflowX: Math.round(rect.right - document.documentElement.clientWidth),
    ...
    if (particles) {
      ...
      if (ctx) {
        ...
        for (let i = 3; i < data.length; i += 4) if (data[i] > 20) particlePixels += 1;
      }
    }
    ...
    if (beams) {
      ...
      if (ctx) {
        ...
        const data = ctx.getImageData(0, 0, w, h).data;
        for (let i = 3; i < data.length; i += 4) if (data[i] > 8) beamPixels += 1;
      }
    }

    const featurePad = Array.from(document.querySelectorAll(".home-feature")).map((el, i) => ({
      i,
      paddingTop: getComputedStyle(el).paddingTop,
      padding: getComputedStyle(el).padding,
    }));

    const reveal = Array.from(document.querySelectorAll(".home-reveal")).map((el) => ({
    ...
    return {
      ...
      particleCanvas: particles
        ? { width: particles.width, height: particles.height, styleWidth: particles.style.width, styleHeight: particles.style.height }
        : null,
...
async function main() {
  ...
  note(response?.ok() ? "info" : "error", "page-load", `status=${response?.status()}`);
  await page.waitForTimeout(900);
  await page.screenshot({ path: join(outDir, "desktop-hero.png"), fullPage: false });

  const hero = await collectPageSignals(page, "hero");
  await page.screenshot({ path: join(outDir, "desktop-full.png"), fullPage: true });

  if (consoleErrors.length) note("error", "console-errors", consoleErrors.slice(0, 8).join(" | "));
  else note("info", "console-errors", "none");
  if (pageErrors.length) note("error", "page-errors", pageErrors.slice(0, 8).join(" | "));
  else note("info", "page-errors", "none");
  const criticalFails = failed.filter((r) => !/favicon|fonts\.googleapis|gstatic|hot-update/i.test(r));
  if (criticalFails.length) note("error", "network-failed", criticalFails.slice(0, 8).join(" | "));
  else note("info", "network-failed", "none");

  if (!hero.h1 || !/Show up/i.test(hero.h1)) note("error", "missing-h1", JSON.stringify(hero.h1));
  if (hero.overflowX > 8) note("error", "overflow-x-hero", `${hero.overflowX}px`);
  if (hero.motion !== "running") note("error", "motion-not-running", String(hero.motion));

  const windowBox = hero.boxes.find((b) => b.selector === ".home-window");
  if (!windowBox || windowBox.width < 200) note("error", "hero-window-missing", JSON.stringify(windowBox));
  if (windowBox && (!windowBox.animationName || windowBox.animationName === "none")) {
  ...
  if (!videoBox || videoBox.height < 40) note("error", "video-box", JSON.stringify(videoBox));
  if (hero.videoState?.error) note("error", "video-error", JSON.stringify(hero.videoState));
  else if (hero.videoState && hero.videoState.videoWidth === 0) note("error", "video-not-decoded", JSON.stringify(hero.videoState));
  else if (hero.videoState?.paused) note("warn", "video-paused", JSON.stringify(hero.videoState));
  else note("info", "video", JSON.stringify(hero.videoState));

  if (hero.particlePixels < 50) note("error", "particles-invisible", `pixels=${hero.particlePixels} canvas=${JSON.stringify(hero.particleCanvas)}`);
  else note("info", "particles", `pixels=${hero.particlePixels}`);
  if (hero.beamPixels < 20) note("warn", "beams-faint", `pixels=${hero.beamPixels} canvas=${JSON.stringify(hero.beamCanvas)}`);
  ...
  for (const check of hero.contrastChecks) {
    ...
    else if (check.ratio < 4.5) note("error", "contrast-fail", `${check.selector} ratio=${check.ratio} color=${check.color} bg=${check.background}`);
  }
  ...
  const navTopBefore = await page.evaluate(() => document.querySelector(".home-nav")?.getBoundingClientRect().top ?? null);
  await page.evaluate(() => window.scrollTo({ top: 1400, behavior: "instant" }));
  await page.waitForTimeout(500);
  const navTopAfter = await page.evaluate(() => document.querySelector(".home-nav")?.getBoundingClientRect().top ?? null);
  await page.screenshot({ path: join(outDir, "desktop-practice.png") });
  if (navTopAfter !== null && Math.abs(navTopAfter) > 4) {
    note("error", "sticky-nav-broke", `before=${navTopBefore} after=${navTopAfter}`);
  } else {
    note("info", "sticky-nav", `before=${navTopBefore} after=${navTopAfter}`);
  }

  const mid = await collectPageSignals(page, "scrolled");
  const unrevealedVisible = mid.reveal.filter((r) => !r.revealed && r.top < 700);
  if (unrevealedVisible.length) note("error", "reveal-stuck", JSON.stringify(unrevealedVisible));
  else note("info", "reveal", JSON.stringify(mid.reveal.map((r) => ({ revealed: r.revealed, top: r.top, opacity: r.opacity }))));

  await page.evaluate(() => document.querySelector("#system")?.scrollIntoView({ behavior: "instant", block: "start" }));
  await page.waitForTimeout(700);
  await page.screenshot({ path: join(outDir, "desktop-system.png") });
  const system = await collectPageSignals(page, "system");
  const featureBoxes = system.boxes.filter((b) => b.selector.startsWith(".home-feature"));
  if (featureBoxes.length !== 3) note("error", "feature-count", String(featureBoxes.length));
  const stillHidden = system.reveal.filter((r) => !r.revealed && r.top < 800);
  if (stillHidden.length) note("error", "system-reveal-stuck", JSON.stringify(stillHidden));

  await page.evaluate(() => document.querySelector(".home-final")?.scrollIntoView({ behavior: "instant", block: "start" }));
  ...
  if (mobile.overflowX > 8) note("error", "overflow-x-mobile", `${mobile.overflowX}px`);
  ...
  for (const check of dark.contrastChecks) {
    if (!check.missing && check.ratio < 4.5) note("error", "dark-contrast-fail", `${check.selector} ratio=${check.ratio}`);
  }

  const brokenImages = [...hero.images, ...dark.images].filter((img) => img.complete && img.naturalWidth === 0);
  if (brokenImages.length) note("error", "broken-images", brokenImages.map((i) => i.src).join(" | "));
